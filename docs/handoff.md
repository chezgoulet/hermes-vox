...
- **Realtime "never listens" — RESOLVED (commit `a043a64`, 0.3.11, 2026-08-27).** Root cause: `autoOpenLine()` gated re-open on a sticky `lineOpen=true` that is never reset, so once the loop stopped (background/turn/walkie) realtime could never restart. Fix: gate on `controller.isListening()` (actual loop state) and drop the sticky flag. Walkie (one-shot PTT) confirmed working in 0.3.10's log; realtime re-listens in-foreground. Still-open design choice: realtime stops on background (onStop); "always-on hands-free" (keep mic hot in background) needs the owner's call.
