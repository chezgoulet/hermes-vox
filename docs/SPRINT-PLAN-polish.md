...
- **Slash commands as native mini-UIs (NOT agent strings):** each `/command` should bring up a **native panel** for its capability, not send a string to the agent to talk about. **API grounding (verified):** the OpenAI `/v1/models` returns ONLY `hermes-agent` — too thin. Use **`GET /api/model/options`** which returns the full gateway catalog `{providers:[{slug,name,models[],total_models,is_current,authenticated,source,capabilities,warning}]}`. Examples (all use the same entity url + bearer key):
  - **`/models` → native model chooser** from `GET /api/model/options` (the full provider/model catalog: `{providers:[{slug,name,models[],total_models,is_current,authenticated,source,capabilities}]}`, with auth/current badges). **Selecting a model swaps the entity's inference backend, NOT the entity** — per the Hermes docs, `/v1/responses` "reconstructs the full conversation from the stored response chain (all tool calls + results preserved)" and "authenticated clients can override Hermes' default model selection per request." So the app sends the chosen **`provider` + `model`** in `/v1/responses` (the blessed per-request override, `_extract_request_runtime_kwargs` → `requested_provider`/`requested_model`), and the entity's memory/skills/context/SOUL persist. **No `/api/sessions/{id}/model` / SessionDB dependency** (that's the messaging/browser-session surface, not Vox's API-client case).
  ...
  - The agent is for *conversation*; the model chooser reads the gateway catalog + sets the per-request provider/model directly (never a `"/models"` string to the agent).
